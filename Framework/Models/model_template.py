...
from utils.smoothing_utils import get_derivatives, get_integrals
...
class model_template():
    ...
    def random_smoothing(self, X, Pred_agents, mode = 'positions', sigma = 1.0, pred_agents_only = True):
        r'''
        This function adds random noise to the input data (randomized smoothing).
        ...
        Pred_agents : np.ndarray
            ...
            if it expected by the framework that a prediction will be made for the specific agent.
        mode : str, optional
            This indicates which data should be perturbed. It can be one of the following:
                'positions'        - only the position data (x and y) is perturbed.
                'position_matched' - only the position data (x and y) is perturbed, and their derivatives (if included in 
                                     *self.input_data_type*) will be adjusted to maintain kinematic consistency.
                'all'              - all data is perturbed.
                'control'          - only the control data (highest derivative in *self.input_data_type*) is perturbed.
                'control_matched'  - only the control data (highest derivative in *self.input_data_type*) is perturbed, and
                                     its integrals will be adjusted to maintain kinematic consistency.
            The default is 'positions'.
        sigma : float, optional
            This is the standard deviation of the Gaussian noise that is added to the data. The default is 1.0.
            The value given is assumed to be in the units of the position data. If controls are perturbed, the value is chosen
            so that for the last input step, the assumed standard deviation of the positions is supposed to be sigma.
        pred_agents_only : bool, optional
            If True, only the agents that are supposed to be predicted (i. e., *self.Pred_agents*) are perturbed. 
            If False, all agents are perturbed. The default is True.

        Returns
        -------
        X_perturbed : np.ndarray
            This is the perturbed past observed data of the agents, in the form of a
            :math:`\{N_{samples} \times N_{agents} \times N_{I} \times N_{data}\}` dimensional numpy array with float 
            values. Here, :math:`N_{data}` are the number of information available.
        '''
        assert mode in ['positions', 'position_matched', 'all', 'control', 'control_matched']
        assert sigma > 0.0, 'The standard deviation (sigma) must be positive.'

        # If positions_matched is chosen, check if derivatives are available.
        if (mode == 'position_matched') and len(self.input_data_type) < 3:
            raise ValueError('Position matched smoothing was chosen, but no derivatives are available in the input data.')
        
        # If control or control matched is chosen, check if control inputs are available
        if (mode in ['control', 'control_matched']) and len(self.input_data_type) < 3:
            raise ValueError('Control smoothing was chosen, but no control inputs are available in the input data.')
        
        # Define sigma state for different input types
        num_samples, num_agents, num_steps = X.shape[:3]
        if 'v' in self.input_data_type:
            i_v = self.input_data_type.index('v')
            P_v = X[...,i_v]
        elif 'v_x' in self.input_data_type and 'v_y' in self.input_data_type:
            i_vx = self.input_data_type.index('v_x')
            i_vy = self.input_data_type.index('v_y')
        ...
        Sigma = {
            'x'       : sigma * ones_value,
            'y'       : sigma * ones_value,
            'v_x'     : sigma / np.sqrt(num_steps) / self.dt * ones_value,
            'v_y'     : sigma / np.sqrt(num_steps) / self.dt * ones_value,
            'v'       : sigma / np.sqrt(num_steps) / self.dt * ones_value,
            'a_x'     : sigma * np.sqrt(3) / np.sqrt(num_steps**3) / self.dt**2 * ones_value,
            'a_y'     : sigma * np.sqrt(3) / np.sqrt(num_steps**3) / self.dt**2 * ones_value,
            'a'       : sigma * np.sqrt(3) / np.sqrt(num_steps**3) / self.dt**2 * ones_value,
            'theta'   : sigma / mean_v / np.sqrt(num_steps),
            'd_theta' : sigma * np.sqrt(3) / mean_v / np.sqrt(num_steps**3),
            }
        
        # Check what states are control states
        # Control inputs are in the following order:
        # if included, first are a_x and a_y, then a and d_theta, then a and theta, then v and d_theta, then v_y and v_x, then v and theta.
        control_states = []
        if 'a_x' in self.input_data_type and 'a_y' in self.input_data_type:
            control_states = ['a_x', 'a_y']
        elif 'a' in self.input_data_type and 'd_theta' in self.input_data_type:
            control_states = ['a', 'd_theta']
        elif 'a' in self.input_data_type and 'theta' in self.input_data_type:
            control_states = ['a', 'theta']
        elif 'v' in self.input_data_type and 'd_theta' in self.input_data_type:
            control_states = ['v', 'd_theta']
        elif 'v' in self.input_data_type and 'theta' in self.input_data_type:
            control_states = ['v', 'theta'] 
        elif 'v_x' in self.input_data_type and 'v_y' in self.input_data_type:
            control_states = ['v_x', 'v_y']
        else:
        ...
        X_perturbed = X.copy()
        if mode == 'all':
            Sigma_all = np.stack([Sigma[s] for s in self.input_data_type], axis=-1) # (num_samples, num_agents, num_data)
            Sigma_all = np.tile(Sigma_all[..., np.newaxis, :], (1, 1, num_steps, 1)) # (num_samples, num_agents, num_steps, num_data)
            X_perturbed += np.random.normal(0.0, Sigma_all)
        ...
        # Do position matching if needed
        if mode == 'position_matched':
            X_perturbed = get_derivatives(X_perturbed, self.dt, self.input_data_type)
        elif mode == 'control_matched':
            X_perturbed = get_integrals(X_perturbed, self.dt, self.input_data_type, control_states)

        original_finite = np.isfinite(X)
        perturbed_finite = np.isfinite(X_perturbed)
        # Wherever original is finite, ensure perturbed is finite as well
        assert perturbed_finite[original_finite].all(), 'Perturbation introduced NaN values where there were none originally.'

        # if only predicted agents are to be perturbed, set other agents to unperturbed values
        if pred_agents_only:
            X_perturbed[~Pred_agents] = X[~Pred_agents]

        # Ensure that angles are projected back onto [-pi, pi]
        if 'theta' in self.input_data_type:
            i_theta = self.input_data_type.index('theta')
            X_perturbed[..., i_theta] = (X_perturbed[..., i_theta] + np.pi) % (2 * np.pi) - np.pi 
    ...
    def _transform_predictions_to_numpy(self, Pred_index, Output_path_pred, 
        ...
        self.Path_true_past = np.zeros((*i_sampl_sort.shape, self.num_timesteps_in, 2), np.float32)
