...
class trajectron_salzmann_smoooth_eval(model_template):
    '''
    ...
    def define_default_kwargs(self):
        ...
        if not ('smoothing_method' in self.model_kwargs.keys()):
            self.model_kwargs['smoothing_method'] = 'positions'
        
        if not ('smoothing_sigma' in self.model_kwargs.keys()):
            self.model_kwargs['smoothing_sigma'] = 0.0
    ...
    def train_method(self, epochs = 100):
        ...
        for epoch in range(1, epochs + 1):
            ...
            while not epoch_done:
                ...
                S, S_St, first_h, Y, Y_st, Neighbor, Neighbor_edge, img_out, node_type = self.extract_data_batch(X, T, Y, img, num_steps)
    ...
    def predict_method(self):
        ...
        while not prediction_done:
            ...
            X, T, _, img, img_m_per_px, _, Pred_agents, num_steps, Sample_id, Agent_id, prediction_done = self.provide_batch_data('pred', batch_size)
            ...
            Pred_final = np.stack(Preds, axis = -1).mean(-1)
            self.save_predicted_batch_data(Pred_final, Sample_id, Agent_id)
    ...
    def get_name(self = None):
        ...
        file_name = 't_pp2_' + smooth_method + smooth_sigma + '_' + str(self.model_kwargs['seed']) + '_' + str(int(self.model_kwargs['predict_ego']))
        names = {'print': 'Trajectron ++ (Old_version - smoothed prediction)',
                 'file': file_name,
                 'latex': r'\emph{T++}'}
        return names
